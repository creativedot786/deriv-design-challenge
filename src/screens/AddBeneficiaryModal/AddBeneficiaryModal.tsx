...
import { Button, CountrySelector, Input, Modal, SegmentedControl, Select } from '../../design-system/components'
...
import styles from './AddBeneficiaryModal.module.css'
...
/**
 * M28 — extracted out of Beneficiaries.tsx so "Add beneficiary" opens
 * the same modal from anywhere (Beneficiaries' own header button,
 * Dashboard's Quick Send CTA) instead of the CTA elsewhere just
 * navigating to the Beneficiaries page and leaving the user to find the
 * button again. Owned by App.tsx, same pattern as SendMoneyModal/
 * RateCheckerModal — one instance, opened via AppOutletContext.
 */
export function AddBeneficiaryModal({ isOpen, onClose, onAdd }: AddBeneficiaryModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={resetAndClose} label="Add beneficiary">
      <div className={styles.formCard}>
        <div className={styles.formHeader}>
          <p className="ds-text-h2">New beneficiary</p>
          <button type="button" className={styles.iconButton} onClick={resetAndClose} aria-label="Close">
            <CloseIcon />
          </button>
        </div>
