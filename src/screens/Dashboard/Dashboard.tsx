...
/**
 * Home. Extracted from the former all-in-one Dashboard in M11 — the
 * sidebar/bottom-nav shell now lives in AppShell, rendered around every
 * routed page. `onSendMoneyClick` comes through router Outlet context
 * (set once in AppShell) rather than as a prop, since this screen no
 * longer has a direct parent that owns the modal state.
 */
export function Dashboard() {
  const { onSendMoneyClick } = useOutletContext<{ onSendMoneyClick: () => void }>()
